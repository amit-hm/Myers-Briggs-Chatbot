...
class lstm_decoder(lstm):
	...
	def forward(self,sources,targets,length,speaker_label,addressee_embed,mode='test'):
		...
		with torch.no_grad():
			if mode=='test':    
				...
				return self.beam_search(start,context,h,c,speaker_label,addressee_embed)
			else: 
				start=self.tembed(targets[:,0])
				pred,h,c=self.decoder(context,h,c,start,speaker_label,addressee_embed)
				...
				for i in range(1,self.params.max_decoding_length):
					pred,h,c=self.decoder(context,h,c,self.tembed(predicted_word),speaker_label,addressee_embed)
					predicted_word = self.sample(pred)
...
class decode_model_2(persona):
	...
	def decode(self, line, addressee_embed = None):
		...
		with torch.no_grad():
			completed_history = self.Model(sources,targets,length,speaker_label,addressee_embed,self.mode)
		return self.OutPut(completed_history)
